...
module.exports = function () {
  ...
  Promise.delay(delay).then(function () {
    ...
    var nonUsedFiles = allFiles.filter(function (el) {
      return usedFiles.indexOf(el) === -1;
    });

    if (options.reportJson && options.reportDir) {
      var logFileName = path.join(options.reportDir, '/', 'report.' + uuid() + '.' + module.parent.parent.filename.replace(new RegExp('/', 'g'), '_') + '.json');
      var _log = {
        info: options.info,
        ...
        usedFiles: usedFiles,
        allFiles: allFiles,
        nonUsedFiles: nonUsedFiles
      };
      return Promise.promisify(fs.writeFile)(logFileName, JSON.stringify(_log, null, 3));
    }
    ...
    if (options.reportDir) {
      var _logFileName = path.join(options.reportDir, '/', 'report.' + uuid() + '.' + module.parent.parent.filename.replace(new RegExp('/', 'g'), '_') + '.log');
      logPromise = Promise.promisify(fs.writeFile)(_logFileName, log.join('\n'));
    } else {
